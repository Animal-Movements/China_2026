...
# To have this run quicker, let's analyze 10% of the data, selected from the tail of the dataset
data <- data %>%
  group_by(ID) %>%
  slice_tail(prop = 0.10)

# Summarize dataset being used
# data %>%
#   group_by(ID) %>%
#   summarise(n_na = sum(is.na(ODBA)),
#             n_records = n(),
#             Prop = n_na / n_records)
...
## ----set-hmm1, message=FALSE, warning=FALSE, echo=TRUE-------------------------------------
