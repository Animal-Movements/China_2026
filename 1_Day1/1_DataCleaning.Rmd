...
Throughout introductory exercises focused on discrete-time analyses, we will use a dataset collected during my graduate work.  The dataset consists of 36 adult white-bearded wildebeest (*Connochaetes taurinus*) fitted with Lotek WildCell^TM^ GPS tracking devices and monitored across three ecosystems (Amboseli Basin, Athi-Kaputiei Plains, and the Greater Maasai Mara) in southern Kenya from 2010-2013.  These data are freely available on [Movebank](https://www.movebank.org/), a global platform for storing, managing, and sharing animal tracking data.  

Known to be particularly important to ecosystem function and diversity, many resident populations of wildebeest have become threatened with extinction over the past few decades.  Study goals were to understand how natural and anthropogenic change impact wildebeest movements.
...
In this exercise, we will:
  ...
  * Import and clean the wildebeest movement dataset for future analyses
  * Summarize and visualize movement path(s)

# Data Preparation
As a first step, we must first clean the dataset before taking any additional analytical steps. This includes filtering the dataset for completeness, identifying duplicate records, removing invalid start or stop dates, identifying positions of poor data quality, and ordering the dataset sequentially.  Nearly all manufacturers report some type of positional quality, often reported as the type of position (e.g., 1D, 2D, or 3D) or Dilution of Position (DOP - horizontal or vertical).  Large DOPs indicate poor positional quality that can be filtered from the dataset.
...
## Load Collar Data
All data for this exercise are freely available via [Movebank](https://www.movebank.org/). [Movebank](https://www.movebank.org/) provides a standardized and secure environment for storing animal movement data, ensuring data quality, and data consistency across animal movement projects. Movebank allows data owners to control access to their tracking data through customizable permissions. Users can choose to make their data fully public, viewable but not downloadable, or completely private—facilitating data sharing while respecting ownership and confidentiality.  

The data used in this exercise have already been downloaded and are available in your `/Data` folder as a `.csv`. Included is a reference file which was also downloaded from [Movebank](https://www.movebank.org/).  The reference file contains additional details (sex, age) about each animal and are common files that often need to be merged with the tracking data.  
...
Since these data are available on [Movebank](https://www.movebank.org/) as open access, we don't need to provide any login details - we simply need to correctly specify the study id of interest.  Downloading directly from [Movebank](https://www.movebank.org/) is particularly useful when data continue to be collected (i.e., are actively streaming) on a study.  This ensures that you are always working with the most up-to-date dataset for analyses.
...
Here, we will clean the data, filter out unnecessary information, and update the timezone to the local timezone.  We will then filter the data to the start/end dates for each animal to match what has been recorded in our reference dataset.  These dates represent the dates that the collar was deployed on the animal. To join the dataframes, we will match records based on a shared primary key (i.e., id). Lastly, we will check on the completeness of the dataset and remove any (grossly) erroneous data points.

**NOTE**: date/time fields are problematic and can be difficult to import.  If problems exist, you may need to properly format the date/time fields first.  See the [lubridate](https://cran.r-project.org/web/packages/lubridate/index.html) package for some instructions and/or the [strptime](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/strptime) package to format your date/time field. Note the format of **YOUR** own dataset (e.g., "%Y-%m-%d %H:%M:%S").  

Also note that some manufacturers put the date and time in separate columns.  If this occurs with your dataset, you'll need to put these columns together to create a timestamp.  For example, `timestamp = ymd_hms(paste(date,time),tz = Timezone1)`.  By using the `tz` argument, you can specify the timezone of the data.
...
## Make it Spatial
There are two main packages in [R](https://cran.r-project.org/) to handle geospatial vector data: [sp](https://cran.csiro.au/web/packages/sp/index.html) and [sf](https://search.r-project.org/CRAN/refmans/sf/html/sf-package.html).

The first package to provide classes and methods for spatial data types in [R](https://cran.r-project.org/) is called [sp](https://cran.csiro.au/web/packages/sp/index.html). Development began in the early 2000s, in an attempt to standardize how spatial data would be treated and to allow for better interoperability between different analysis packages that use spatial data. The package (first released on CRAN in 2005) provides classes and methods to create points, lines, polygons, and grids, with functions to work with them.  About 350 packages developed for spatial analysis use the data types implemented in [sp](https://cran.csiro.au/web/packages/sp/index.html), which means several spatial packages “depend” on the [sp](https://cran.csiro.au/web/packages/sp/index.html) package in some way.

The [sp](https://cran.csiro.au/web/packages/sp/index.html) package depends on `rgeos` and `rgdal`. However, at the end of 2023, these packages were retired, primarily because their maintainer, Roger Bivand, also retired. You can read more about it [here](https://r-spatial.org/r/2022/04/12/evolution.html).

All geospatial work in [R](https://cran.r-project.org/) is being replaced by the more modern package [sf](https://search.r-project.org/CRAN/refmans/sf/html/sf-package.html). [sf](https://search.r-project.org/CRAN/refmans/sf/html/sf-package.html) was first released on CRAN in late October 2016. It implements a formal standard called “Simple Features” that specifies a storage and access model of spatial geometries (i.e. points, lines, polygons).  [sf](https://search.r-project.org/CRAN/refmans/sf/html/sf-package.html) provides fast I/O, particularly relevant for large files.

## The [sf](https://search.r-project.org/CRAN/refmans/sf/html/sf-package.html) Package
...
[sf](https://search.r-project.org/CRAN/refmans/sf/html/sf-package.html) objects consist of rows of features, hence the name “simple features ([sf](https://search.r-project.org/CRAN/refmans/sf/html/sf-package.html))”, which have both non-spatial and spatial data formats.  The spatial data of an [sf](https://search.r-project.org/CRAN/refmans/sf/html/sf-package.html) object is contained in a special geometry column that is of class ‘sfc - simple features class’. The geometry column contains the basic types of spatial data: the Coordinate Reference System (CRS), coordinates, and type of geometric object.

The ‘sfc’ class has seven subclasses to denote the types of geometric objects within the geometry column, which are derived from the simple features standard. The possible geometric objects are point, linestring, polygon, multipoint, multilinestring, multipolygon, and geometry collection, which is used for any combination of the other types.

![Artwork by [Allison Horst](https://allisonhorst.com/allison-horst)](Figures/AllisonHorst_sf.png)

<br>

## Convert a Dataframe to a [sf](https://cran.r-project.org/web/packages/sf/index.html) Object
...
1. Import your movement dataset
