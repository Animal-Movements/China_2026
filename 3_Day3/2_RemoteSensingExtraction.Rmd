...
Because our rasters are saved as a list, we will use the `do.call()` function to `mosaic()` the grids together and use `mean` for potential overlapping areas, which isn't necessary here.  As a result, we could use the `merge()` function instead.
