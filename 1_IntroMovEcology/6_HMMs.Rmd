...
There are two main packages for fitting discrete-time hidden Markov models (HMMs) in [R](https://cran.r-project.org/): [moveHMM](https://cran.r-project.org/web/packages/moveHMM/index.html) and [momentuHMM](https://cran.r-project.org/web/packages/momentuHMM/index.html).  Both packages are maintained by the package architects, Theo Michelot and Brett McClintock.  The difference between the two packages is the complexity of models that can be fitted, with [moveHMM](https://cran.r-project.org/web/packages/moveHMM/index.html) limited to estimating 2 or so biologically-meaningful behavior states from the data stream (i.e,. step length, turning angle). [MomentuHMM](https://cran.r-project.org/web/packages/momentuHMM/index.html), however, allows for the incorporation of auxiliary biotelemetry information (e.g., from pressure, conductivity, heart rate, or motion sensors), which results in less restrictions on the constraints to parameters and provides for greater possibilities in estimating additional behavioral states.
...
In this exercise, we will:
  ...
  * Practice methods for visualizing HMM output both spatially and graphically [MAYBE]
...
Load the cleaned wildebeest dataset from the previous exercise.   In terms of data preparation, a few processing/formatting steps to note:

  * Data should have negligible location error.  If you are working with high levels of error (common to marine species that are being tracked by Argos) or data are irregular, you should likely process it first before analysis. For example, you may try a state space approach, such as [crawl](https://cran.r-project.org/web/packages/crawl/index.html) or [aniMotum](https://ianjonsen.github.io/aniMotum/articles/Overview.html), or use [ctmm](https://cran.r-project.org/web/packages/ctmm/index.html) to correct for the GPS error and use `predict()` to create the most likely path of your animal. [MomentuHMM](https://cran.r-project.org/web/packages/momentuHMM/index.html) also provides a 2-stage imputation approach when data are temporarily-irregular or subject to measurement error.  See the `help()` for more information.
  * The animal id column must be named "ID".
  * It also seems very helpful to convert the coordinates (currently in meters) to km by dividing by 1000.
...
To begin modeling, we need to set starting values for all relevant estimable parameters.  This is arguably the most challenging aspect of model fitting.  [MomentuHMM](https://cran.r-project.org/web/packages/momentuHMM/index.html) includes a number of functions, including `GetPar`, `getPar0`, `getParDM`, and `checkPar0`, to help with specification of starting values.  See the `help()` for tips and tricks.   

As a starting point, we will fit a 2-state (NULL) model, without specifying covariates to influence behavioral transitions.  We will then build a few more complex models where the covariate(s) inform the transition from one state to the next.  The first step is to specify the number of behavioral states we want to identify, which require us to provide initial values for the vectors stepPar0 (for the step distributions) and anglePar0 (for the angle distributions).  We then need to select a statistical distribution that best describe each of these vectors.  

[MomentuHMM](https://cran.r-project.org/web/packages/momentuHMM/index.html) includes a variety of distributions.  We will use the gamma distribution to describe steplengths and the von Mises for turning angles. Other common distributions for steplength include Weibull ("weibull"), exponential ("exp"), and log-normal ("lnorm").  Wrapped-Cauchy ("wrpcauchy") is commonly used as an alternative distribution to describe the turning angles.  You can experiment with other distributions to find the best fitting model. 
...
# Print confidence intervals.  Specify the significance level.
CI(WB.temp)
...
# Fit with random effects on individual.  More Complicated model.
# WB.temp.ran <- fitHMM(data = WB.move, 
#                       nbStates = 2, 
#                       stepPar0 = stepPar0, 
#                       anglePar0 = anglePar0,
#                       stepDist = "gamma",
