...
There are a number of [R](https://cran.r-project.org/) packages that can be used to convert our dataframe to an animal movement trajectory.  We will use the [adehabitatLT](https://cran.r-project.org/web/packages/adehabitatLT/index.html) and [amt - animal movement tools](https://cran.r-project.org/web/packages/amt/index.html) packages for all analyses described here.  The [move2](https://cran.r-project.org/web/packages/move2/index.html) package also has additional features that are worth exploring.  All packages differ in the commands and workflow used, but will provide the same end product for further analyses.  Please see the vignettes included with each package for further instruction.

I am most familiar with [adehabitatLT](https://cran.r-project.org/web/packages/adehabitatLT/index.html) package and tend to default to using it for animal movement analyses.  [Amt](https://cran.r-project.org/web/packages/amt/index.html), however, has grown in its popularity over the last few years, using tracks as it basic building block.  One of its key strengths lies in its tidy, modular approach to construct movement workflows. The coding structure lends itself well when moving to more complicated analyses. 
...
In this exercise, we will:
  ...
  * Investigate the [adehabitatLT](https://cran.r-project.org/web/packages/adehabitatLT/index.html) and [amt](https://cran.r-project.org/web/packages/adehabitatLT/index.html) packages for analyzing animal movement data
...
#install.packages('adehabitatLT', 'amt', 'sf', 'tidyverse', 'unit', 'gridExtra', 'lubridate', 'date', 'magick')
...
The GPS collar manufacturer we are using here (Lotek Wireless) provides data with a DOP measure. A good recommendation is to place your GPS device in an open constellation and in a stationary location before fitting the device on an animal.  This will provide validation data required to assess GPS error (i.e., How much GPS scattering exists?).  The error associated with positions is often device or even batch specific.  Here, I’ll show a very basic way of filtering poor quality data points, although many other options exist (e.g., speed-based filtering).  Always be careful when filtering out data from further analysis.
...
# Export
write_rds(WB.traj.reg, file = "Data/wildebeest_WB_Hourly.rds")
```
...
#**Note**, you must you double brackets [[]] because the output is a list
#names(WB.traj.reg[[1]])
#head(WB.traj.reg[[1]])
...
We can also create our own custom plots, summarizing the movement trajectory or the summary information on how animals moved, using commands demonstrated in the previous exercise. This gives us greater flexibility to explore specific items.  Here, I provide a simple example of a movement trajectory that I commonly create, which includes graphing the animal points, the distance moved, and how far the animal moved from an initial location (NSD - Net Squared Displacement).  We will learn more about NSD in the next lecture.

This code could be edited slightly to loop over each individual and plot results that are specific to each animal, or animate the movement trajectory.  I highly recommend plotting your data and spending time looking at each animal's movement trajectory.  
...
# This same workflow could be adopted using GGPLot (A good homework assignment!)
...
time.diff <- WB.sub %>% 
  summarize(start = min(date),
            ...
            #diff = end - start)
            diff = trunc(difftime(end, start, units="days"))) %>%  # difftime allows me to specify the units; trunc will truncate the output to an integer 
...
We can adopt this code easily to "loop" across each individual, outputting results to a directory of our choosing.  See the 'Output/Plots' folder to view the graph created for each individual.  I am using a 'for' loop here, but the `map` function could be used.  See `help(map)` for more information.

```{r Plotting Loop, message=FALSE, warning=FALSE, echo=TRUE}
...
## Custom Plotting - Animate the graph

We can now take this a step further and animate the result.  Here, will create an animation of the entire trajectory on an daily time step.  To do so, we need to create a graph for every day that the animal was tracked and output a figure that can be incorporated into an animated gif.  If creating the outputs using [ggplot2}(https://cran.r-project.org/web/packages/ggplot2/index.html), you could also use `gganimate`.  See `help(gganimate)` for more information.

```{r Animate, message=FALSE, warning=FALSE, eval=FALSE, echo=TRUE}
# Set Output Directory
...
for (i in 1:length(Days.unique)){
  ...
  # Create plots
	plot(WB.sub$x,WB.sub$y,type="n",lwd=1.5,xlim=c(min.X,max.X),ylim=c(min.Y,max.Y),xlab="Easting",ylab="Northing",asp=1,bty="n",axes=FALSE,frame.plot=TRUE,main=Title1)
	mtext(paste(format(WB.sub$date[1],"%Y-%m-%d")," to ",
	            format(WB.sub$date[nrow(WB.sub)],"%Y-%m-%d")),
	      cex=0.75)
	...
	# Plot the movements over time (Velocity)
  plot(WB.sub$date,WB.sub$dist/1000,type="n",lwd=1.5,xlim=c(min.Time,max.Time),ylim=c(min.dist,max.dist),bty="n",xlab="Days Tracked",ylab="Velocity (km/hr)",axes=TRUE,frame.plot=TRUE,main=Title2) # Remember that this is a 3 hour dataset
	lines(Day.sub$date, Day.sub$dist/1000, type='l', col="gray") # Graph all data in gray
	lines(Day.new$date, Day.new$dist/1000, type='l', col="blue") # Graph new data in blue
	# Calculate the time from release date	
	mtext(paste(time.diff,"days"),cex=0.75)

	# Plot the net displacement per step
	plot(WB.sub$date,sqrt(WB.sub$R2n)/1000,type="n",lwd=1.5,xlim=c(min.Time,max.Time),ylim=c(min.NSD,max.NSD),xlab="Days Tracked",ylab="Distance (km)",bty="n",axes=TRUE,frame.plot=TRUE,main=Title3) 
	lines(Day.sub$date, sqrt(Day.sub$R2n)/1000, type='l', ylab="Distance (km)", xlab="Days Tracked", col="gray") # Graph all data in gray
	lines(Day.new$date, sqrt(Day.new$R2n)/1000, type='l', col="blue") # Only the new data
	mtext(paste(time.diff,"days"),cex=0.75)

dev.off()
}

# Now use ImageMagick to convert the .png to a .gif
```
...
Now we will perform the same procedure, except using the functions from the [Amt](https://cran.r-project.org/web/packages/adehabitatLT/index.html) package.

```{r AMT Track, message=FALSE, warning=FALSE, echo=FALSE, eval=FALSE}
