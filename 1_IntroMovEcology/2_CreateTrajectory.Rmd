...
for (i in 1:length(Days.unique)){
  ...
  # Create plots
	plot(WB.sub$x,WB.sub$y,type="n",lwd=1.5,xlim=c(min.X,max.X),ylim=c(min.Y,max.Y),xlab="",ylab="",asp=1,bty="n",axes=FALSE,frame.plot=TRUE,main=Title1)
	...
	# Plot the date
	text(min.X+15000,max.Y-10000, paste("Date: ",format(Date.txt, "%d/%m/%Y")),pos=4,cex=1.5)
	
	# Plot the movements over time (Velocity)
  plot(WB.sub$date,WB.sub$dist/1000,type="n",lwd=1.5,xlim=c(min.Time,max.Time),ylim=c(min.dist,max.dist),bty="n",xlab="",ylab="",axes=TRUE,frame.plot=TRUE,main=Title2, cex=2) 
	...
	# Plot the net displacement per step
	plot(WB.sub$date,sqrt(WB.sub$R2n)/1000,type="n",lwd=1.5,xlim=c(min.Time,max.Time),ylim=c(min.NSD,max.NSD),xlab="",ylab="",bty="n",axes=TRUE,frame.plot=TRUE,main=Title3) 
