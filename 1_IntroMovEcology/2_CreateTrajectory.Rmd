...
There are a number of [R](https://cran.r-project.org/) packages that can be used to convert our dataframe to an animal movement trajectory.  We will use the [adehabitatLT](https://cran.r-project.org/web/packages/adehabitatLT/index.html) and [amt](https://cran.r-project.org/web/packages/amt/index.html) packages for all analyses described here.  The [move2](https://cran.r-project.org/web/packages/move2/index.html) package also has additional features that are worth exploring.  All packages differ in the commands and workflow used, but will provide the same end product for further analyses.  Please see the vignettes included with each package for further instruction.

The [Amt - Animal Movement Tools](https://cran.r-project.org/web/packages/amt/index.html) has grown in its popularity over the last few years, using tracks as it basic building block.  One of its key strengths lies in its tidy, modular approach to constructing movement workflows. The coding structure, however, can take some time to get used to, but lends itself well when moving to the next stages of analyses, importantly which include calculating movement metrics, generate random steps, and fitting integrated step selection functions (iSSFs). 
...
In this exercise, we will:
  ...
  * Clean the animal movement trajectories and make the track 'regular'
...
#install.packages('adehabitatLT', 'amt', 'sf', 'tidyverse', 'unit', 'gridExtra')
...
**Note**, these values are specific to our dataset.
...
# AdeHabitatLT requires a dataframe for analyses, which means we need to remove the geographic information that was added to the dataframe when converting to a sf object.  We'll use the WB.sf dataset here because the data have been projected (i..e, we need projected coordinates for analyses.  Thus, let's convert to a flat' dataframe.
...
# Look at the data
# How does this file differ from the WB.sf object?
head(WB.data)
```
...
The GPS collar manufacturer we are using here (Lotek Wireless) provides data with a DOP measure. A good recommendation is to always place your GPS device in an open constellation and in a stationary location before fitting the device on an animal.  This will provide the validation data required to assess GPS error/scattering.  The error associated with positions is often device or even batch specific.  Here, I’ll show a very basic way of filtering poor quality data points, although many other options exist (e.g., speed-based filtering).  Always be careful when filtering out data from further analysis.
...
WB.data <- 
  WB.data %>% 
  filter(
    fixType == "3D" & DOP < 10 | fixType == "2D" & DOP < 5) # Only accept position with a 3D fixtype and DOP less than 10 or a 2D fixtype and DOP less than 5
...
We can also plot the trajectory of each individual, creating a simple loop to output a plot to a directory of our choosing.  This requires a small change to the code created above.  See the 'Output/Plots' folder to view the graph created for each individual.
