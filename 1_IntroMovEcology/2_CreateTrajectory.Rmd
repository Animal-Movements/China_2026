...
The [Amt](https://cran.r-project.org/web/packages/amt/index.html) has grown in its popularity over the last few years, using tracks as it basic building block.  One of its key strengths lies in its tidy, modular approach to construct movement workflows. The coding structure, however, can take some time to get used to, but lends itself well when moving to the next stages of analyses, which importantly includes calculating movement metrics, generate random steps, and fitting integrated step selection functions (iSSFs). 
...
## Data Import
Load the data from the previous exercise. Recall that the `.Rdata` file includes two files, `WB` and `WB.sf`.  The `WB.sf` file is essentially the same as the `WB` object, except that the data are projected (i.e., the data have spatial coordinates).  Let's use this file since we are ultimately interested in understanding how animals (i.e., calculating metrics of movement between points). 
...
WB.data <- WB.sf %>%
  as_tibble() %>%
  mutate(Easting = st_coordinates(WB.sf)[,1],
         Northing = st_coordinates(WB.sf)[,2]) %>%
