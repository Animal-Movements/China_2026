...
The [Amt](https://cran.r-project.org/web/packages/amt/index.html) has grown in its popularity over the last few years, but using tracks as it basic building block.  This can take some time to get used to, but lends itself well when moving to the next stages of analyses, importantly which include calculating integrated step selection functions (iSSFs). 
...
# AdeHabitatLT requires a dataframe for analyses, which means we need to remove the geographic information that was added to the dataframe by converting to a sf object.  We'll use the WB.sf dataset here because the data have been projected (i..e, we need projected coordinates for analyses.  Thus, let's convert to a flat' dataframe.
