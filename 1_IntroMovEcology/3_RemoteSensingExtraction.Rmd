...
We will load the [sf](https://search.r-project.org/CRAN/refmans/sf/html/sf-package.html) that we created in the "1_DataCleaning" lecture.  These data have been project to UTM Zone 37 South. As a result, we will need to reproject the data before extracting the elevation data at each data point. 

**Please Note:** It is usually a best practive to reproject spatial points to match a raster's projection because interpolation methods will distort the grid cells and (potentially) change the values.  This means that projecting the raster files back and forth can lead to compounding errors. 
...
Load the [sf](https://search.r-project.org/CRAN/refmans/sf/html/sf-package.html) object created in the previous exercise. Note that the `WB.sf` object was saved after being project to UTM Zone 37 South (`EPSG:32727`). We will need to reproject these data to decimal degrees (`EPSG:4326`) to extract the raster values at each point location.  
...
Because our rasters are saved as a list, we will use the `do.call()` function to `mosaic()` each grid together.  Note, we will use the `mean()` function for overlapping areas.  We could similarly use the `merge()` function if no overlap exists between grids (likely the case here).
...
# Exercise
Raster data layers can become quite large and difficult to handle in memory.  In these cases, it's often better to process each raster in a stepwise fashion, rather than mosaicking or merging them together.  

How would you cycle through each raster, extracting the values from each, before moving on to the next raster in sequence? 
