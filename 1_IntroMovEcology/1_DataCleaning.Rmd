...
#install.packages('tidyverse', 'move2', 'lubridate', 'tmap', 'sf', 'gt')
...
# Since these data are available on Movebank as open access, we don't need to provide any login details - we simply need to correctly specify the study id of interest.  Downloading directly from Movebank is particularly useful when data continue to be collected (i.e., are actively streaming) on a study.  This ensures that you are always working with the most up-to-date dataset being collected.
...
Many of the columns included in the dataframes are unnecessary (and quite long).  We need to clean and filter the dataframes. To do so, we will use 'piping' (e.g., %>%) from the [tidyverse](https://cran.r-project.org/web/packages/tidyverse/index.html) package.  This allows us to combine multiple sequential commands together.
...
WB <- # Note that this will overwrite the existing dataset
  WB %>% 
  ...
  mutate(id = tag_local_identifier,
         ...
         # Create local timestamp (UTC to local) using 'with_tz' command
