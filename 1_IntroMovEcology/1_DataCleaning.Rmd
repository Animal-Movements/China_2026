...
NNP <- st_read("Data/NNP_DDWGS84.shp") %>% 
...
Athi.Map <-
  ...
  tm_shape(WB.sf,
    ...
    tm_dots(col = "id", # COlor based on the id
            palette = viridis::viridis(length(unique(WB.sf$id))), # A color palette
            size = 0.001, # Size of the points
            legend.show = TRUE) +
  
  # Load the wildebeest data - Lines
  tm_shape(WB.lines,
           name = "Wildebeest Lines") +
    tm_lines(lwd = 0.25, # Controls the width of the lines
             col = 'id',
             palette = viridis::viridis(length(unique(WB.lines$id))),
