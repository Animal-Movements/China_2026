...
# filter to more accurate data points
WB <- WB %>%
  as_tibble() %>%
...
# first create a summary table for the dataset 
WB_summary <- WB %>%
...
# nest the tracks to prep for metrics calculation 
...
## ----metrics-functions---------------------------------------------------------
# calculating displacement at the given temporal scale 
calc_disp <- function(track_df, time_unit = c("day", "week")) {
  
  time_unit <- match.arg(time_unit)
  
  # for each temporal group, calculate distance between the first and the last locations (i.e. displacement)
  track_df %>%
...
calc_max_disp <- function(track_df, time_unit = c("month", "year")) {
  ...
  # if temporal period is year, calculate maximum displacement for the entire track
  if (time_unit == "year") {
    ...
    # otherwise, group the dataset and calculate maximum displacement for each group
    df <- track_df %>%
      mutate(period = floor_date(t_, unit = time_unit)) %>%
      group_by(period) %>%
      group_split()
...
# calculate monthly or yearly track straightness
...
# calculate monthly or yearly intensity of use, calculated by dividing the total movement distance (tot_dist) by the square of the area of movement (= minimum convex polygon 100).
...
# calculate monthly and yearly MCP 
...
# monthly 95% MCP of use per animal
...
# ward.D2 creates compact, spherical clusters (often fits behavioral mode clustering).
# Visualization
...
kmeans_result <- kmeans(metrics.mat, centers = 3, nstart = 25) # when centers is a number (the number of clusters), a random set of (distinct) rows in x is chosen as the initial centres. nstart defines this initial set of rows. 
metrics.df$cluster <- as.factor(kmeans_result$cluster) # turn cluster ID into factor
