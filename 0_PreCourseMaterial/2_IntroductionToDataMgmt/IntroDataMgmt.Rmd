...
* Further your understanding of [R](https://cran.r-project.org/) and the [RStudio](https://posit.co/downloads) interface
...
Now that you've learned the basics of [R](https://cran.r-project.org/) programming, we'll take things a step further and start working on your skills related to data analysis.  You will likely be unfamiliar with some of the operations you need to execute in this exercise. Part of the goal with this exercise, however, is for you to become more familiar with the *help* commands in [R](https://cran.r-project.org/) and with the internet solutions that exist.  Our ultimate goal is to make you aware of the tools that are available so that you can become an effective problem solver, working independently on data analyses.

# Running Code in a Script

So far, we've run code in the Console.  This is fine for quick queries. For anything to be shareable, reproducible, and to reduce our own efforts in the long-term, we should run code via a script.  A [R](https://cran.r-project.org/) script is a just a simple text file.  [RStudio](https://posit.co/downloads) uses the script by copying [R](https://cran.r-project.org/) commands from text in the file and pastes them into the Console as if you were manually entering the commands yourself.  This greatly enhances our ability to build off what we have created in the past, learn from previous experience, and quickly re-run analyses when new data are received.  To create an [R](https://cran.r-project.org/) script:

1. From the "file" mean, select "New File"
2. Click "R Script" from the list of options

[RStudio](https://posit.co/downloads) will open your [R](https://cran.r-project.org/) script automatically after creating it.  Notice that the scripting window appears above the Console in what's known as the **Source** pane.

3. Save the [R](https://cran.r-project.org/) script in your working directly in the `Scripts` folder.  Name this file `IntroDataMgmt.R`. 

![Figure 1: Create a script for running analyses](Figures/Fig1.png)
<br>
...
Running code via an [R](https://cran.r-project.org/) script is different than running code in the Console.  To interpret and run the code you've written, [R](https://cran.r-project.org/) needs you to send the code from the script to the Console.  Some common ways to run your code include:
...
The most basic [R](https://cran.r-project.org/) skill is to query and manipulate data tables. As a beginner programmer, it is imperative to familiarize yourself with how to manipulate data.  Reinforcing these skills is like expanding your vocabulary in the new language that you are learning and is a great way to improve your [R](https://cran.r-project.org/) proficiency. If you wish to become really good at [R](https://cran.r-project.org/), but don't know where to start, start with data table manipulation! 

<div style="float:right">
<img width="150" height="150" src="Figures/dplyr.jpg">
</div>

The base [R](https://cran.r-project.org/) functions that come with the default [R](https://cran.r-project.org/) installation have the capacity for almost all the table manipulation needs (e.g., `split(), subset(), apply(), sapply(), lapply(), tapply(), aggregate()`). However, sometimes their syntax are less user-friendly and intuitive than some of the special packages built for table manipulation purposes. Here, we are introducing a few of the most useful table manipulation functions within the `dplyr` package. 
...
[R](https://cran.r-project.org/) has multiple functions for reading in table data.  Here we'll use the base function `read.csv()` to import a table named `panda_data.csv` that is located in your `Data` folder. Text files (`.txt`) can be imported using the function `read.delim()`.  See the help files for each function and search [Google](https://www.google.com/) for information on other functions to read other data types.
...
![Artwork by [Allison Horst](https://allisonhorst.com/allison-horst)](Figures/dplyr_filter.png)
...
![Artwork by [Allison Horst](https://allisonhorst.com/allison-horst)](Figures/dplyr_mutate.png)
...
![Artwork by [Allison Horst](https://allisonhorst.com/allison-horst)](Figures/dplyr_relocate.png)
...
**Note**: Sometimes the column headings between tables do not match, even if the data within the column does (`ID` in table 1 != `id` in table 2).  In these cases, you can either `rename()` the column headings to make them match or specify the columns you want to join within the function.
...
Now that you've been exposed to a few functions to better manage data in [R](https://cran.r-project.org/), we will reinforce these tools by asking you to apply them to memory by practicing on a dataset of wildlife counts collected across the Naboisho conservancy.  Located in your `Data` folder and named `SampleData.csv`, these data are derived from a publication in the African Journal of Ecology.

<div style="float:right">
<img width="150" height="150" src="Figures/DataScience.png">
</div>
