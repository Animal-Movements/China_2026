...
**Periodogram:** Plot showing the log spectral density (akin to energy or activity) of the data (y-axis) across the period (x-axis). The period ($\frac{1}{frequency}$) is the timescale of the data. If we were analyzing acoustic data, white noise would produce a flat line in the periodogram (same energy at every frequency). Periodograms allow us to identify autocorrelated movement across timescales, which are characteristic of patterns in movement periodicity (periodic space use). For example, an albatross that hunts once each month would show a spike in activity at the one month mark.
...
In this section we will first “fit” models the variograms and then perform model selection across various continuous-time models. This "fit" is not a rigorous statistical fitting, but a good way of choosing candidate movement models and guessing at their parameters. The variogram-fit parameter guess estimates will then be fed into maximum likelihood estimation, which requires good initial guesses for non-linear optimization.
...
The next step is to fit and select the best continuous-time movement model for our data. While each model is tested, the likelihood function is optimized to find th parameter values that best fit the data, starting from our initial values stored in GUESS.

We use the `ctmm.select` function for AICc-based model selection. It will fit a bunch of autocorrelation and continuous-time models, tell us what models are being fit, and return the top models based on $\Delta$AICc. This process can be slow, but it can be parallelized to save time if your operating system allows for it (e.g., Unix systems), using all but one CPU core by default. The computation time is linearly proportional to the sample size: 10x more data = 10x longer to run.
...
> * Ornstein-Uhlenbeck $\Omega$ "omega" (OU$\Omega$) <!-- <-- CONFIRM THIS MODEL -->
...
* Degrees of freedom (DOF): effective sample size (*more to come*)
  * DOF[area] = number of data points
...
By default, `ctmm.select` fits a number of nested continuous-time movement models under the assumption of range residency (`ctmm(range = TRUE)`: OU and OUF, both isotropic and anisotropic. But, we can force non-resident models if our tracking data does not appear range resident (autocorrelation persists).
...
_Next: **Module 4 — `ctmm`-Based Movement Metrics** — We use continuous-time methods to estimate movement speed, distance, and diffusion._
