...
Do the residuals look normally distributed? <span style="color: white;"> **(NO!)** </span>
...
* **Area (DOF[area], $N_{\text{area}}$):** ESS data points we have to estimate the home-range area. 
  * Note the smaller effective sample size -- only ~18 data points due to autocorrelation.
* **Diffusion (DOF[diffusion], $N_{\text{diffusion}}$):** ESS data points we have to estimate the diffusion.
...
* **Area:** Gaussian area, this represents the estimated # home-range crossings (and its lower and upper CI bounds).
...
FITS2 <- ctmm.select(DATA, GUESS2, verbose = TRUE, trace = FALSE)
