...
> **Discussion:** Look at where each behavior type clusters along the fence line rather than just at the overall mix of behaviors — how might that spatial pattern translate into a concrete conservation recommendation (e.g., where to prioritize a wildlife-friendly fence retrofit)?
...
# load the 3h regular data 
load("Data/WB_3h_resampled.rdata")
WB <- trk_resampled_3h %>%
  rename(Animal.ID = id,
         date = t_) %>%   # BaBA requires an "Animal.ID" column and a "date" timestamp column
  #drop_na(x, y) %>% 
  st_as_sf(., coords = c("x_", "y_"), crs = st_crs(32737))
...
mapView(WB.lines, zcol = "Animal.ID", layer.name = "Wildebeest", cex = 1) +
...
WB.sf.2ind <- WB %>%
  filter(Animal.ID %in% c("Ntishya", "Sotua")) %>%
  arrange(Animal.ID, date)
