---
title: "Applying clustering algorithms on movement metrics"
...
Because we will calculate the same metrics across all individuals at different temporal intervals (day, week, or year), we can define the functions first then apply to the tracks using `map`. 
```{r metrics-functions}
...
With the functions defied, we can now compute all the metrics by applying the functions to each track of the nested track through `map`. 
```{r compute-metrics}
...
Combine all metrics into a single data frame, prepping for clustering later.
```{r data combo}
...
Heirachical clusting with `ward.D2` distance.
...
K-means clustering, which aims to partition the points into k groups such that the sum of squares from points to the assigned cluster centres is minimized.
```{r k-means}
...
|           <span style="color:tomato">**QUESTION: ** From the track maps we created before, can you try to interpret the clustering results? Is 3 a seasonable number of clusters? Try different clustering numbers and see whether results are different.  </span>
<br/><br/>
